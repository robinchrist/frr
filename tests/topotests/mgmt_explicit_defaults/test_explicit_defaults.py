...
def test_with_defaults_view(tgen):
    ...
    # The CLI rendering of both views, via the vtysh grammar and directly
    # against mgmtd (which owns the datastore).
    for daemon in (None, "mgmtd"):
        path = daemon or "vtysh"
        plain_cli = r1.vtysh_cmd("show configuration running", daemon=daemon)
        effective_cli = r1.vtysh_cmd(
            "show configuration running with-defaults", daemon=daemon
        )
        assert " default-metric 1" in plain_cli.splitlines(), (
            "explicit default-metric missing from plain CLI view (%s)" % path
        )
        assert not any(
            "timers basic" in l for l in plain_cli.splitlines()
        ), "implicit timers in plain CLI view (%s)" % path
        assert " timers basic 30 180 120" in effective_cli.splitlines(), (
            "implicit timers missing from with-defaults CLI view (%s)" % path
        )

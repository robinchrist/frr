...
.. clicmd:: route-target <import|export|both> <RTLIST|auto>
   ...
   An export Route Target will not be attached to routes that are learned from other peers that are re-advertised.
   ``both`` applies the list to import and export.
...
.. clicmd:: route-target <import|export|both> <RTLIST>
   ...
   Configure the route-target set for EVPN for a specific MAC-VRF / L2VNI (
   the terms are equivalent for FRR, because FRR uses the "VLAN-Based Service Interface" model,

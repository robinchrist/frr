...
/* free wrapper struct */
void bgp_evpn_rt_config_free(struct bgp_evpn_rt_config* config) {
 ...
 */
int evpn_route_select_install(struct bgp *receiving_vrf, struct bgp_evpn_evi *evi,
			      struct bgp_dest *dest, struct bgp_path_info *pi)
{
	...
	if (CHECK_FLAG(dest->flags, BGP_NODE_SELECT_DEFER)) {
		if (BGP_DEBUG(graceful_restart, GRACEFUL_RESTART))
			zlog_debug("%s: SELECT_DEFER flag set for EVPN route %pBD, dest %p",
				   receiving_vrf->name_pretty, dest, dest);
	...
	bgp_best_selection(receiving_vrf, dest, &receiving_vrf->maxpaths[afi][safi], &old_and_new,
			   afi, safi);
	...
	if (old_select && old_select == new_select
	    ...
	    && !bgp_addpath_is_addpath_used(&receiving_vrf->tx_addpath, afi, safi)) {
		if (bgp_zebra_has_route_changed(old_select)) {
			/* BP is disabled when  BGP instance is being deleted or
			 * GR is in progress.
			 */
			if (CHECK_FLAG(receiving_vrf->flags, BGP_FLAG_DELETE_IN_PROGRESS) ||
			    CHECK_FLAG(receiving_vrf->gr_info[afi][safi].flags, BGP_GR_SKIP_BP))
				ret = evpn_zebra_install(receiving_vrf, evi,
							 (const struct prefix_evpn *)
								 bgp_dest_get_prefix(dest),
							 old_select);
			else
				bgp_zebra_route_install(dest, old_select, receiving_vrf,
							true, evi, false);
		}
	...
	if (new_select && new_select->type == ZEBRA_ROUTE_BGP
		...
		if (CHECK_FLAG(receiving_vrf->flags, BGP_FLAG_DELETE_IN_PROGRESS) ||
			...
			bgp_zebra_route_install(dest, new_select, receiving_vrf, true,
						evi, false);
		...
		if (new_select->sub_type == BGP_ROUTE_IMPORTED &&
				old_select && old_select->peer == receiving_vrf->peer_self
				&& old_select->type == ZEBRA_ROUTE_BGP
				&& old_select->sub_type == BGP_ROUTE_STATIC
				&& evi)
			evpn_delete_old_local_route(receiving_vrf, evi, dest,
					old_select, new_select);
	} else {
		if (old_select && old_select->type == ZEBRA_ROUTE_BGP &&
		    old_select->sub_type == BGP_ROUTE_IMPORTED) {
			if (CHECK_FLAG(receiving_vrf->flags, BGP_FLAG_DELETE_IN_PROGRESS) ||
			    CHECK_FLAG(receiving_vrf->flags, BGP_FLAG_VNI_DOWN) ||
			    CHECK_FLAG(receiving_vrf->gr_info[afi][safi].flags, BGP_GR_SKIP_BP))
				ret = evpn_zebra_uninstall(receiving_vrf, evi,
							   (const struct prefix_evpn *)
								   bgp_dest_get_prefix(dest),
							   old_select, false);
			else
 ...
 */
static int bgp_evpn_evi_update_route_entry(struct bgp *bgp_evpn_mi, struct bgp_evpn_evi *evi,
				   afi_t afi, safi_t safi,
				   ...
				   uint32_t seq, bool vpn_rt, bool *old_is_sync)
{
	...
	*pi = NULL;
	...
	local_pi = bgp_evpn_route_get_local_path(bgp_evpn_mi, dest, 0);
	 ...
	 */
	update_evpn_route_entry_sync_info(bgp_evpn_mi, dest, attr, seq, vpn_rt, mac);
 ...
 */
static int bgp_evpn_evi_update_route(struct bgp *bgp_evpn_mi, struct bgp_evpn_evi *evi,
			     ...
			     uint32_t seq, esi_t *esi)
{
	...
	bgp_attr_default_set(&attr, bgp_evpn_mi, BGP_ORIGIN_IGP);
	...
	route_change = bgp_evpn_evi_update_route_entry(
		bgp_evpn_mi, evi, afi, safi, dest, &attr,
	 ...
	 */
	evpn_route_select_install(bgp_evpn_mi, evi, dest, pi);
	/*
	...
	if (CHECK_FLAG(pi->flags, BGP_PATH_REMOVED)) {
		...
		if (!CHECK_FLAG(pi->flags, BGP_PATH_SELECTED)) {
			route_change = 0;
			dest = evpn_cleanup_local_non_best_route(bgp_evpn_mi, evi, dest,
								 pi);
		} else {
			bool new_is_sync;
			...
			if (!new_is_sync && old_is_sync) {
				if (CHECK_FLAG(bgp_evpn_mi->flags,
					       BGP_FLAG_DELETE_IN_PROGRESS))
					evpn_zebra_uninstall(bgp_evpn_mi, evi, p, pi,
							     true);
				else
					bgp_zebra_route_install(dest, pi, bgp_evpn_mi,
								false, evi,
								true);
			}
		}
	...
	if (route_change) {
		...
		/* Schedule for processing and unlock node. */
		bgp_process(bgp_evpn_mi, dest, global_pi, afi, safi);
		bgp_dest_unlock_node(dest);
	}
...
void bgp_evpn_evi_update_type2_route_entry(struct bgp *bgp_evpn_mi, struct bgp_evpn_evi *evi,
				       struct bgp_dest *dest,
				       struct bgp_path_info *local_pi,
				       const char *caller)
{
	...
	route_change = bgp_evpn_evi_update_route_entry(
		bgp_evpn_mi, evi, afi, safi, dest, &attr, NULL /* mac */, NULL /* ip */,
	...
	if (CHECK_FLAG(pi->flags, BGP_PATH_REMOVED)) {
		...
		if (!CHECK_FLAG(pi->flags, BGP_PATH_SELECTED)) {
			route_change = 0;
			evpn_cleanup_local_non_best_route(bgp_evpn_mi, evi, dest, pi);
		} else {
			bool new_is_sync;
			...
			if (!new_is_sync && old_is_sync) {
				if (CHECK_FLAG(bgp_evpn_mi->flags,
					       BGP_FLAG_DELETE_IN_PROGRESS))
					(void)evpn_zebra_uninstall(bgp_evpn_mi, evi,
								   &evp, pi,
								   true);
				else
					bgp_zebra_route_install(dest, pi, bgp_evpn_mi,
	...
	if (route_change) {
		...
		/* Schedule for processing and unlock node. */
		bgp_process(bgp_evpn_mi, global_dest, global_pi, afi, safi);
		bgp_dest_unlock_node(global_dest);
	}
...
static void bgp_evpn_evi_update_type2_route(struct bgp *bgp_evpn_mi, struct bgp_evpn_evi *evi,
			       struct bgp_dest *dest)
{
	struct bgp_path_info *tmp_pi;
	...
	tmp_pi = bgp_evpn_route_get_local_path(bgp_evpn_mi, dest, 0);
	if (!tmp_pi)
		return;

	bgp_evpn_evi_update_type2_route_entry(bgp_evpn_mi, evi, dest, tmp_pi, __func__);
}
 ...
 */
void bgp_evpn_evi_update_all_type2_routes(struct bgp *bgp_evpn_mi, struct bgp_evpn_evi *evi)
{
	struct bgp_dest *dest;
	...
	for (dest = bgp_table_top(evi->mac_table); dest;
		...
		bgp_evpn_evi_update_type2_route(bgp_evpn_mi, evi, dest);
 ...
 */
int bgp_evpn_evi_update_type_1_2_3_routes(struct bgp *bgp_evpn_mi, struct bgp_evpn_evi *evi)
{
	struct prefix_evpn p;

	update_type1_routes_for_evi(bgp_evpn_mi, evi);
	...
	if (bgp_evpn_evi_get_flood_mode(bgp_evpn_mi, evi) == VXLAN_FLOOD_HEAD_END_REPL) {
		...
		bgp_evpn_evi_update_route(bgp_evpn_mi, evi, &p, 0, 0, NULL);
	}
 ...
 */
static int bgp_evpn_evi_delete_routes(struct bgp *bgp_evpn_mi, struct bgp_evpn_evi *evi)
{
	int ret;
	struct prefix_evpn p;
	 ...
	 */
	bgp_evpn_evi_delete_all_type2_routes(bgp_evpn_mi, evi);
	 ...
	 */
	SET_FLAG(bgp_evpn_mi->flags, BGP_FLAG_VNI_DOWN);
	ret = bgp_evpn_evi_delete_route(bgp_evpn_mi, evi, &p);
	UNSET_FLAG(bgp_evpn_mi->flags, BGP_FLAG_VNI_DOWN);
	if (ret)
		return ret;

	/* Delete all routes from the per-VNI table. */
	bgp_evpn_evi_delete_all_routes(bgp_evpn_mi, evi);
	return 0;
}
...
static int install_evpn_route_entry_in_vni_common(
	struct bgp *receiving_vrf, struct bgp_evpn_evi *evi, const struct prefix_evpn *p,
	struct bgp_dest *dest, struct bgp_path_info *parent_pi)
{
	...
	ret = evpn_route_select_install(receiving_vrf, evi, dest, pi);
	 ...
	 */
	local_pi = bgp_evpn_route_get_local_path(receiving_vrf, dest, 0);
	if (local_pi && (old_local_es || new_local_es))
		bgp_evpn_evi_update_type2_route_entry(receiving_vrf, evi, dest, local_pi,
						  __func__);
 ...
 */
static int bgp_evpn_evi_install_route_entry_ip(struct bgp *receiving_vrf,
					      struct bgp_evpn_evi *evi,
					      const struct prefix_evpn *p,
					      struct bgp_path_info *parent_pi)
{
	...
	ret = install_evpn_route_entry_in_vni_common(receiving_vrf, evi, p, dest,
						     parent_pi);
 ...
 */
static int bgp_evpn_evi_uninstall_route_entry_ip(struct bgp *receiving_vrf,
						struct bgp_evpn_evi *evi,
						const struct prefix_evpn *p,
						struct bgp_path_info *parent_pi)
{
	...
	ret = uninstall_evpn_route_entry_in_vni_common(receiving_vrf, evi, p, dest,
						       parent_pi);
 ...
 */
static int
bgp_evpn_evi_uninstall_route_entry_mac(struct bgp *receiving_vrf, struct bgp_evpn_evi *evi,
				      const struct prefix_evpn *p,
				      struct bgp_path_info *parent_pi)
{
	int ret;
 ...
 */
static int bgp_evpn_evi_install_route_entry(struct bgp *receiving_vrf, struct bgp_evpn_evi *evi,
				    const struct prefix_evpn *p,
				    struct bgp_path_info *parent_pi)
{
	int ret = 0;
	...
	if (bgp_debug_update(parent_pi->peer, NULL, NULL, 1) || bgp_debug_zebra(NULL))
		zlog_debug(
			...
			vrf_id_to_name(receiving_vrf->vrf_id), receiving_vrf->vrf_id, p, evi->vni);

	tmp.family = p->family;
	tmp.prefixlen = p->prefixlen;
	tmp.u.prefix_evpn = p->prefix;
	prefix2str(&tmp, prefix_str, sizeof(prefix_str));
	frrtrace(4, frr_bgp, upd_evpn_route_entry, 1, receiving_vrf->vrf_id, prefix_str, evi->vni);

	ret = bgp_evpn_evi_install_route_entry_mac(receiving_vrf, evi, p, parent_pi);
	...
	ret = bgp_evpn_evi_install_route_entry_ip(receiving_vrf, evi, p, parent_pi);
 ...
 */
static int bgp_evpn_evi_uninstall_route_entry(struct bgp *receiving_vrf, struct bgp_evpn_evi *evi,
				      const struct prefix_evpn *p,
				      struct bgp_path_info *parent_pi)
{
	int ret = 0;
	...
	tmp.u.prefix_evpn = p->prefix;
	prefix2str(&tmp, prefix_str, sizeof(prefix_str));
	frrtrace(4, frr_bgp, upd_evpn_route_entry, 0, receiving_vrf->vrf_id, prefix_str, evi->vni);

	ret = bgp_evpn_evi_uninstall_route_entry_ip(receiving_vrf, evi, p, parent_pi);
	...
	ret = bgp_evpn_evi_uninstall_route_entry_mac(receiving_vrf, evi, p, parent_pi);
...
static void advertise_withdraw_type3(struct bgp_evpn_evi *evi, struct bgp *bgp)
{
	...
	if (!evi || !bgp_evpn_evi_is_live(evi))
		return;
 ...
 */
void bgp_evpn_vrf_configure_rd(struct bgp *bgp_vrf, struct prefix_rd *rd,
 ...
 */
void bgp_evpn_vrf_unconfigure_rd(struct bgp *bgp_vrf)
{
	/* If we have already advertise type-5 routes with a different RD, we
	 * have to delete and withdraw them first
	 */
...
/*
 * Configure RD for an EVI
 */
void bgp_evpn_evi_configure_rd(struct bgp *bgp, struct bgp_evpn_evi *evi,
...
/*
 * Unconfigure RD for an EVI
 */
void bgp_evpn_evi_unconfigure_rd(struct bgp *bgp, struct bgp_evpn_evi *evi)
{
	/* If the VNI is "live", we need to delete and withdraw this VNI's
 ...
 */
void bgp_evpn_evi_derive_auto_rd(struct bgp *bgp_evpn_mi, struct bgp_evpn_evi *evi)
{
	char buf[BGP_EVPN_PREFIX_RD_LEN];

	evi->prd.family = AF_UNSPEC;
	evi->prd.prefixlen = 64;
	snprintfrr(buf, sizeof(buf), "%pI4:%hu", &bgp_evpn_mi->router_id, evi->rd_id);
 ...
 * Lookup VNI.
 */
struct bgp_evpn_evi *bgp_evpn_lookup_evi_by_vni(struct bgp *bgp_evpn_mi, vni_t vni)
{
	struct bgp_evpn_evi *evi;
		...
		enum bgp_evpn_evi_origin origin)
{
	...
	bgp_evpn_evi_derive_auto_rd(bgp_evpn_mi, evi);
 ...
 * This mostly just frees appropriate memory (e.g. also releases from the evihash,
 * unlinks from the Tenant VRF, etc..).
 * If e.g. routes need to be deleted and withdrawn, caller is expected to handle this!
 */
void bgp_evpn_evi_delete_and_free(struct bgp *bgp_evpn_mi, struct bgp_evpn_evi *evi)
{
	...
	/* Caller is expected to delete / withdraw routes */
	bgp_evpn_evi_unmap_from_evi_irt_nodes(evi);
	...
	bgp_evpn_effective_wildcard_rt_slu_fini(&evi->effective_wildcard_import_rts);
	...
	bgp_evpn_effective_fq_rt_slu_fini(&evi->effective_fq_import_rts);

	while((eff_fq_rt = bgp_evpn_effective_fq_rt_slu_pop(&evi->effective_fq_export_rts))) {
		bgp_evpn_effective_fq_rt_free(eff_fq_rt);
	}
	bgp_evpn_effective_fq_rt_slu_fini(&evi->effective_fq_export_rts);
...
/*
 * Cleanup EVPN information on disable, i.e. when this BGP instance stops being the
 * master instance 
 * Need to delete and withdraw EVPN routes from peers.
 * Delete all autoconfigured EVIs (EVIs that are not user configured and
 * were learned from the kernel / through Zebra)
 *

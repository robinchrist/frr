...
extern int bgp_evpn_es_route_install_uninstall(struct bgp *bgp,
		...
		int install);
extern void update_type1_routes_for_evi(struct bgp *bgp, struct bgp_evpn_evi *evi);
extern int delete_global_ead_evi_routes(struct bgp *bgp, struct bgp_evpn_evi *evi);
...
bgp_evpn_remote_es_evi_add(struct bgp *bgp, struct bgp_evpn_evi *evi,
			   const struct prefix_evpn *p,
			   struct bgp_path_info *pi);
extern enum zclient_send_status
bgp_evpn_remote_es_evi_del(struct bgp *bgp, struct bgp_evpn_evi *evi,
			   const struct prefix_evpn *p,
			   struct bgp_path_info *pi);

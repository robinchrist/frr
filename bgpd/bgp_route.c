					     ...
					     struct bgp_path_info *old_select)
{
	...
	if (bgp_evpn_should_originate_type5_routes_bestpath(bgp, afi) && new_select &&
		...
		if (bgp->adv_cmd_rmap[afi][safi].map) {
			...
			if (ret == RMAP_DENYMATCH) {
				...
				bgp_evpn_vrf_upsert_prefix_as_type5_route(bgp, new_select, p, &dummy_attr,
							       afi, safi, 0);
		} else {
			     ...
			     struct peer *peer, afi_t afi, safi_t safi, struct prefix_rd *prd)
{
	...
	if (bgp_evpn_should_originate_type5_routes_multipath(peer->bgp, afi) && is_route_injectable_into_evpn(pi))
		...
		struct bgp_route_evpn *evpn)
{
	...
	if (pi) {
		...
		if (bgp_evpn_should_originate_type5_routes_multipath(bgp, afi) &&
			...
			bgp_evpn_vrf_inject_prefix_and_originate_as_type5_route(bgp, dest, pi, afi, safi);
		}
		...
		if (safi == SAFI_EVPN) {
			if ((!same_attr || force_evpn_import) &&
				...
				bgp_evpn_import_global_received_route(bgp, afi, safi, p, pi);
	...
	if (safi == SAFI_EVPN && CHECK_FLAG(new->flags, BGP_PATH_VALID))
		bgp_evpn_import_global_received_route(bgp, afi, safi, p, new);
	/* If this is a route we should export for EVPN, process for export */
	if (bgp_evpn_should_originate_type5_routes_multipath(bgp, afi) && is_route_injectable_into_evpn(new))
		bgp_evpn_vrf_inject_prefix_and_originate_as_type5_route(bgp, dest, new, afi, safi);
...
filtered:
	...
	if (pi) {
		...
		if (bgp_evpn_should_originate_type5_routes_multipath(bgp, afi) && is_route_injectable_into_evpn(pi))
			bgp_evpn_unexport_type5_route(bgp, dest, pi, afi, safi);
			...
			for (pi = bgp_dest_get_bgp_path_info(dest);
				...
				if (safi == SAFI_UNICAST &&
					...
					vpn_leak_from_vrf_withdraw(
						...
						pi);
				if (bgp_evpn_should_originate_type5_routes_multipath(peer->bgp, afi) &&
				    is_route_injectable_into_evpn(pi))
...
void bgp_cleanup_routes(struct bgp *bgp)
{
	...
	FOREACH_AFI_SAFI (afi, safi) {
		 ...
		 */
		if (safi != SAFI_MPLS_VPN && IS_BGP_INSTANCE_AUTO(bgp) && !bm->terminating)
			continue;
		...
		for (pi = bgp_dest_get_bgp_path_info(dest); (pi != NULL) && (next = pi->next, 1);
			...
			if (pi->extra && pi->extra->aggr_suppressors &&
				...
				if (aggr_unsuppress_path(aggregate, pi)) {
					...
					if (bgp_evpn_should_originate_type5_routes_multipath(bgp, afi) &&
					    is_route_injectable_into_evpn(pi))
						bgp_evpn_vrf_inject_prefix_and_originate_as_type5_route(bgp, dest, pi, afi,
									    safi);
					    ...
					    const struct prefix *aggr_p)
{
	...
	if (aggregate->summary_only && AGGREGATE_MED_VALID(aggregate))
		if (aggr_unsuppress_path(aggregate, pi)) {
			...
			if (bgp_evpn_should_originate_type5_routes_multipath(bgp, afi) &&
			    is_route_injectable_into_evpn(pi))
				bgp_evpn_vrf_inject_prefix_and_originate_as_type5_route(bgp, pi->net, pi, afi, safi);
			  ...
			  route_tag_t tag)
{
	...
	if (red) {
		...
		if (bpi) {
			...
			if (!CHECK_FLAG(bpi->flags, BGP_PATH_REMOVED) &&
				...
				if (bgp_evpn_should_originate_type5_routes_multipath(bgp, afi) &&
					...
					bgp_evpn_vrf_inject_prefix_and_originate_as_type5_route(bgp, bn, bpi, afi,
								    SAFI_UNICAST);
		...
		if (bgp_evpn_should_originate_type5_routes_multipath(bgp, afi) &&
		    is_route_injectable_into_evpn(new))
			bgp_evpn_vrf_inject_prefix_and_originate_as_type5_route(bgp, bn, new, afi, SAFI_UNICAST);
		    ...
		    enum rpki_states rpki_target_state, bool brief)
{
	...
	if (bgp == NULL || IS_BGP_INSTANCE_AUTO(bgp)) {
		if (!use_json)
			  ...
			  bool use_json, uint16_t show_opts)
{
	if (!bgp) {
		bgp = bgp_get_default();
		if (!bgp || IS_BGP_INSTANCE_AUTO(bgp)) {
			if (!use_json)
				vty_out(vty, "No BGP process is configured\n");
			else
				vty_out(vty, "{}\n");

...
/* 'bgp default shutdown' (M8 batch B2). Legacy emits this line AFTER the
 * neighbor lines precisely so a restart's file replay does not shut peers
 * that already exist (FRR #2286); the converted leaf keeps those
 * future-peers-only semantics by only setting bgp->autoshutdown (seeded
 * into each subsequent peer_create), never touching existing peers, and
 * by carrying BGP_NB_PRIO_INSTANCE_DEFAULT_SHUTDOWN (bgp_nb.c) so a
 * config-load transaction replays it after the neighbor CREATEs. A
 * hand-written file that placed the line before its neighbors behaves as
 * if it were written after them - the machine-emitted order, and the
 * leaf's documented intent. */

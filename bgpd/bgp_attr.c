...
/* bgp_path_attribute_discard_vty()/bgp_path_attribute_withdraw_vty(): the
 * legacy CLI-layer setters for discard_attrs[]/withdraw_attrs[] below,
 * retired along with their sole callers (the neighbor_path_attribute_*
 * DEFPYs, bgp_vty.c) when this feature was converted to northbound (M4
 * batch B14, see bgpd/proteus/bgp_nb_neighbor.c and bgp_nb_peer_group.c).
 * peer->discard_attrs[]/withdraw_attrs[] themselves remain in active use by
 * bgp_attr_ignore() below and its callers.
 */

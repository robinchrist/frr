...
route_set_srte_color(void *rule, const struct prefix *prefix, void *object)
{
	...
	bgp_path_info_extra_get(path)->srte_color = *srte_color;
...
{
	/*
	 * bgpd is a backend daemon (M3 B-RM1): do not install lib's
	 * northbound route-map CLI locally. All frr-route-map/
	 * frr-bgp-route-map CLI (including bgpd's own match/set
	 * commands, bgp_routemap_cli.c) now runs only in mgmtd,
	 * exactly like ripd/zebra/staticd; bgpd receives the applied
	 * config over the mgmt_be_client backend channel instead
	 * (/frr-route-map:lib in bgpd_config_xpaths[], bgp_main.c).
	 */

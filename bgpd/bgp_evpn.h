...
evpn_zebra_uninstall(struct bgp *bgp, struct bgp_evpn_evi *vpn,
...
extern void bgp_zebra_evpn_pop_items_from_announce_fifo(struct bgp_evpn_evi *vpn);
extern int install_uninstall_routes_for_vni(struct bgp *bgp, struct bgp_evpn_evi *vpn, bool install);
